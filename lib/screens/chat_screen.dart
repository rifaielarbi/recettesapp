...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _onSend() async {
    ...
    String reply;
    try {
      // Essayer d'utiliser l'API AI
      reply = await _aiService.ask(text);
      
      // Si la réponse indique que l'API n'est pas configurée, utiliser le fallback
      if (reply.contains('Clé API manquante') || reply.contains('Clé API invalide')) {
        reply = _offlineAnswer(text);
      }
    } catch (e) {
      // En cas d'erreur, utiliser le fallback local
      debugPrint("Erreur AI Service: $e");
      reply = _offlineAnswer(text);
    }
